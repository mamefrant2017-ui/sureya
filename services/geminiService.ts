// Use exact import from @google/genai as per guidelines.
...
export async function getAIAdvisorResponse(prompt: string, history: {role: string, parts: {text: string}[]}[]) {
  try {
    // Initialization must always use a named parameter and the exact property from process.env.API_KEY.
    ...
    // The text content is accessed via the .text property of the GenerateContentResponse object.
